...
const Telemedicine = () => {
  ...
  const fetchSession = async () => {
    try {
      ...
      setError('Could not find a valid telemedicine session. Ensure payment is complete.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const startMeeting = async () => {
    ...
    const options = {
      ...
      userInfo: {
        displayName: JSON.parse(localStorage.getItem('user'))?.name || 'User',
      },
    };
      ...
      <div className="flex-grow relative overflow-hidden flex flex-col lg:flex-row">
        ...
        <div className="flex-grow relative bg-black">
          {!isJoined ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center space-y-8 bg-slate-950">
              ...
              <div className="flex items-center space-x-4">
                <button 
                  onClick={() => setControls(c => ({...c, mic: !c.mic}))}
                  className={`p-4 rounded-2xl transition-all shadow-xl ${controls.mic ? 'bg-slate-800 text-white' : 'bg-red-500 text-white shadow-red-500/20'}`}
                >
                  {controls.mic ? <Mic className="w-6 h-6" /> : <MicOff className="w-6 h-6" />}
                </button>
                <button 
                  onClick={() => setControls(c => ({...c, video: !c.video}))}
                  className={`p-4 rounded-2xl transition-all shadow-xl ${controls.video ? 'bg-slate-800 text-white' : 'bg-red-500 text-white shadow-red-500/20'}`}
                >
                  {controls.video ? <Video className="w-6 h-6" /> : <VideoOff className="w-6 h-6" />}
                </button>
                <button 
                  onClick={startMeeting}
                  className="btn btn-primary !px-12 !py-4 text-lg shadow-2xl shadow-primary-600/20"
                >
                  Join Meeting
                </button>
              </div>
        ...
        <div className="hidden lg:flex w-80 bg-slate-900 flex-col border-l border-slate-800">
           <div className="p-6 border-b border-slate-800">
             <h4 className="text-white font-bold uppercase tracking-widest text-xs mb-6">Patient Records</h4>
             <div className="space-y-4">
                <div className="p-4 bg-slate-800/50 rounded-2xl border border-slate-700/50 group hover:border-primary-500/30 transition-all">
                  <p className="text-[10px] text-primary-500 font-bold uppercase mb-1">Previous Diagnosis</p>
                  <p className="text-sm text-slate-300 font-medium leading-relaxed">Mild Seasonal Allergies</p>
                </div>
                <div className="p-4 bg-slate-800/50 rounded-2xl border border-slate-700/50 group hover:border-primary-500/30 transition-all">
                  <p className="text-[10px] text-primary-500 font-bold uppercase mb-1">Last Prescription</p>
                  <p className="text-sm text-slate-300 font-medium leading-relaxed">Cetirizine 10mg - OD</p>
                </div>
             </div>
           </div>
           ...
           <div className="flex-grow p-6 flex flex-col justify-end space-y-6">
              ...
              <button className="w-full btn btn-outline !border-slate-800 !text-slate-400 hover:!bg-slate-800 flex items-center justify-center space-x-2">
